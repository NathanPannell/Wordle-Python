...
# Thank you to cfreshman on GitHub for providing well organized data for me to use :) Guesses: ~12000 5-letter words
# Answers: ~2000 common 5-letter words The answer should always be a somewhat common word. The program will accept
# all 12000 5-letter english words as valid guesses.
GUESSES = 'guesses.txt'
ANSWERS = 'answers.txt'
...
def game() -> None:
    ...
    answer_letters = {}
    for char in answer:
        add_to_dict(char, answer_letters)
    ...
    while tries < MAX_TRIES:
        ...
        for i in range(5):
            if answer[i] == guess[i]:
                tiles = change_char_at_index(GREEN_TILE, i, tiles)
                add_to_dict(answer[i], matches)
        ...
        for i in range(5):
            ...
            if char in answer_letters:
                ...
                if char not in matches:
                    matches[char] = 1
                    tiles = change_char_at_index(YELLOW_TILE, i, tiles)
                ...
                # If every instance of char in answer has been matched already, further instances of char in guess
                # are left grey
...
def add_to_dict(value, dictionary: dict) -> None:
    """
...
def change_char_at_index(character: str, index: int, string: str) -> str:
    """
...
def get_input_in_guesses(message: str) -> str:
    """
...
if __name__ == '__main__':
    ...
    guesses_list = read_file(GUESSES)
    answers_list = read_file(ANSWERS)
